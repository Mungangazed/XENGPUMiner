...
1. **VAST.AI Mining - vast.ai (verified)**:
   ...
   For 4 X GPU use vast4.sh:
      ```
   sudo apt update && sudo apt -y install wget && sudo wget https://github.com/Mungangazed/XENGPUMiner/blob/main/vast5.sh && sudo chmod +x vast4.sh && sudo ./vast4.sh
      ```      

   For 8 X GPU use vast8.sh:
      ```
   sudo apt update && sudo apt -y install wget && sudo wget https://github.com/Mungangazed/XENGPUMiner/blob/main/vast8.sh && sudo chmod +x vast8.sh && sudo ./vast8.sh
      ```
...
1. **RUNPOD Mining - runpod.io (unverified)**:
  - Same as above, just update `runpod.sh` file with your address
   ```
   apt update && apt -y install wget && wget https://github.com/Mungangazed/XENGPUMiner/blob/main/runpod.sh && chmod +x runpod.sh && ./runpod.sh
   ```
